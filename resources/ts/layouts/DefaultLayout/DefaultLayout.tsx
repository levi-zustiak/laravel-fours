import {createContext, createEffect, createSignal, JSXElement, onMount, Show} from 'solid-js';
...
export function DefaultLayout(props: { children: JSXElement }) {
    ...
    onMount(() => {
        timeline([
            [initialContainer, { width: 0 }, { delay: 1.25, duration: 1.5, easing: [0.87, 0, 0.13, 1] }],
            [navigation, { transform: "translateX(0)" }],
            [account, { opacity: [0, 1], right: "64px", transform: "translateX(0)" }, { at: "<"}],
  ...
  return (
    <div class={styles.container}>
      ...
      <main class={styles.main}>
        <div ref={content} class={styles.content}>{props.children}</div>
      </main>
    </div>
  );
}
